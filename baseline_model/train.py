        ...
        for batch in train_loader:
            ...
            try:
                ...
                if not torch.isfinite(loss):
                    log(f" Non-finite loss {loss.item()} at step {global_step}")
                    log(f"  audio min/max:   {audio.min():.3f} / {audio.max():.3f}")
                    log(f"  targets min/max: {targets.min()} / {targets.max()}")
                    sys.exit(1)
                ...
                log(f" RuntimeError at step {global_step}: {e}")
                log(traceback.format_exc())
                log(f"  GPU allocated: {torch.cuda.memory_allocated(device)/1e9:.2f}GB")
                log(f"  GPU reserved:  {torch.cuda.memory_reserved(device)/1e9:.2f}GB")
                log(f"  audio shape:   {audio.shape}")
                log(f"  targets shape: {targets.shape}")
                sys.exit(1)
            ...
            # Logging and train loss recording
            ...
            if global_step <= 10 or global_step % LOG_EVERY == 0:
                ...
                gpu_util    = get_gpu_util(0)

                log(
                    f"Step {global_step}/{args.total_steps} | "
                    f"Loss: {loss.item():.4f} | PPL: {ppl:.2f} | "
                    f"Grad: {grad_norm:.3f} | LR: {current_lr:.2e} | "
                    f"data: {(t_data_ready-t_step_end)*1000:.0f}ms "
                    f"xfer: {(t_transfer-t_data_ready)*1000:.0f}ms "
                    f"fwd: {(t_forward-t_transfer)*1000:.0f}ms "
                    f"bwd: {(t_backward-t_forward)*1000:.0f}ms | "
                ...
                # Record train loss at same frequency as logging
            ...
            # Validation: also triggers incremental metrics save
            ...
            if global_step % VALIDATE_EVERY == 0:
                ...
                save_metrics()
                log(f"Metrics saved ({len(train_losses)} train, {len(val_losses)} val entries)")

            if global_step % GREEDY_EVERY == 0:
                evaluate_greedy()

           
            # Checkpointing: metrics embedded in checkpoint for resume
